...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { nickname, promoCode } = payload;
    if (!nickname) {
    ...
    const params = new URLSearchParams({
      ...
      'quickpay-form': 'shop',
      targets: `Покупка VIP статуса SamuraiWorld для ${cleanNick}`,
      paymentType: 'AC',
      sum: amount.toFixed(2),
      label: orderId,
      successURL: `https://samuraiworld.ru/store?payment=success&type=vip&nickname=${encodeURIComponent(cleanNick)}&order=${encodeURIComponent(orderId)}`,
    });
    ...
    return res.status(200).json({
      ...
      message: 'Ссылка на оплату VIP через ЮMoney сгенерирована',
    });
  }
